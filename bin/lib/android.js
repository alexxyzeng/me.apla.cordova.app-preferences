...
module.exports = function(context) {
  ...
  function mapConfig(config) {
    ...
    if (!mapping) throw 'no mapping for ' + config.type;
    ...
    if (mapping.attrs) {
      for (var attrName in mapping.attrs) {
        ...
        if (attrConfig.value) {
          if (
            !attrConfig.value[config[attrName]] ||
            !attrConfig.value[config[attrName]][platformName]
          )
            throw 'no mapping for type: ' +
              config.type +
              ', attr: ' +
              attrName +
              ', value: ' +
          ...
          if (targetAttr) element.attrs[targetAttr].push(config[attrName]);
          else element[elementKey] = config[attrName];
        }
      }
    }
  ...
  function build(config) {
    ...
    return (
      fs
        .exists('platforms/android')
        // Write preferences xml file
        .then(function() {
          return fs.mkdir('platforms/android/res/xml');
        })
        .then(function() {
          return fs.writeFile(
            'platforms/android/res/xml/apppreferences.xml',
            preferencesDocument.write()
          );
        })

        // Write localization resource file
        .then(function() {
          return fs.mkdir('platforms/android/res/values');
        })
        .then(function(prefs) {
          return fs.writeFile(
            'platforms/android/res/values/apppreferences.xml',
            preferencesStringDocument.write()
          );
        })
      ...
      fs
        ...
        .then(function() {
          return fs.readFile(
            path.resolve(
              __dirname,
              '../../src/android/AppPreferencesActivity.template'
            )
          );
        })
        .then(function(tmpl) {
          ...
          var packageName =
            projectConfig.android_packageName() || projectConfig.packageName();
        ...
        .then(function(data) {
          ...
          var activityFileName = path.join(
  ...
  function clean(config) {
    ...
    return (
      fs
        ...
        .then(function() {
          return fs.unlink('platforms/android/res/xml/apppreferences.xml');
        })

        // Remove localization resource file
        .then(function(prefs) {
          return fs.unlink('platforms/android/res/values/apppreferences.xml');
        })
        ...
        .then(function() {
          console.log('android preferences file was successfully cleaned');
        })
        .catch(function(err) {
          if (err.code === 'NEXIST') {
            console.log('Platform android not found: skipping');
